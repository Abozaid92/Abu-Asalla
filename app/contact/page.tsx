  ...
  return (
    <Shell>
      <section className="relative mx-auto w-[min(1180px,calc(100%-48px))] overflow-hidden pt-[105px] pb-[70px] max-[760px]:pt-[65px] max-[760px]:pb-[45px]">
        <Reveal>
          <p className="mb-[17px] text-xs font-semibold tracking-[0.05em] text-[var(--accent-warm)]">تواصل معنا</p>
        ...
        <div className="mt-10 grid grid-cols-2 gap-[26px] max-[760px]:grid-cols-1">
          <Reveal as="div" className="reveal-1">
            <a className={contactCard} href={`https://wa.me/${number}`} target="_blank" rel="noopener">
              ...
              <p className="m-0 text-sm text-[var(--text-2)]" dir="ltr">+{number}</p>
          ...
          {settings.instagramUrl && (
            <Reveal as="div" className="reveal-3">
              <a className={contactCard} href={settings.instagramUrl} target="_blank" rel="noopener">
                <span className={contactIcon}>
                  <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.6">
                ...
                <p className="m-0 text-sm text-[var(--text-2)]">شاهد أحدث أعمالنا يوميًا</p>
          ...
          <Reveal as="div" className="reveal-4">
            <div className={contactCard}>
              ...
              <p className="m-0 text-sm text-[var(--text-2)]">نخدم مدينة الرياض وضواحيها</p>
              {settings.workingHours && <p className="m-0 text-sm text-[var(--text-2)]">{settings.workingHours}</p>}
            </div>
          </Reveal>
        </div>
        ...
        <Reveal
          as="div"
          className="reveal-5 mt-[26px] rounded-[var(--radius-project-sm)] border border-[var(--border)] bg-[var(--surface)] p-[26px]"
        >
          <div className="mb-4">
            <p className="mb-[17px] text-xs font-semibold tracking-[0.05em] text-[var(--accent-warm)]">موقعنا</p>
            <h3 className="mt-1.5 text-[17px]">الرياض، حي الدار البيضاء، شارع عرفات</h3>
          </div>
      ...
      <section className="bg-[var(--accent)] py-[105px] text-white max-[760px]:py-[60px]">
        <div className="mx-auto w-[min(1180px,calc(100%-48px))]">
          <Reveal
            as="div"
            className="rounded-[var(--radius-project)] bg-[var(--accent)] p-11 text-white max-[760px]:p-6 [&_input]:border [&_input]:border-white/15 [&_input]:bg-white/[0.06] [&_input]:text-white [&_label]:text-[#d7d4cd] [&_select]:border [&_select]:border-white/15 [&_select]:bg-white/[0.06] [&_select]:text-white [&_textarea]:border [&_textarea]:border-white/15 [&_textarea]:bg-white/[0.06] [&_textarea]:text-white"
          >
            <p className="mb-[17px] text-xs font-semibold tracking-[0.05em] text-[#d5b188]">أرسل تفاصيل مشروعك</p>
            <h2 className="mt-0 text-[clamp(37px,4.4vw,61px)]">
              خلّنا نجهزلك
              <br />
              عرض سعر دقيق.
            </h2>
            <p className="text-[#a9a6a0]">الاسم، رقم الجوال، ونوع الخدمة — وسنعاود التواصل معك سريعًا.</p>
